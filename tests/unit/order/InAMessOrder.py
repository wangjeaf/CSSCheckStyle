...
def doTest():
    ...
    equal(fixer.doCompress(), "a.feed-back-v6{display:block;position:fixed;_position:absolute;top:155px;_top:expression(documentElement.scrollTop + 'px');right:0;_margin-top:155px;padding:10px;width:1em;background:#F7F7FF;font-size:14px;font-weight:bold;z-index:1999}", 'compress feed-back-v6 is ok')
    ...
    equal(rules[6].name, 'margin-top', 'element name is ok')
    equal(rules[7].name, 'padding', 'element name is ok')
    equal(rules[8].name, 'width', 'element name is ok')
