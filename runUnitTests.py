...
'''单元测试运行器
   将扫描tests目录下的所有py和css文件
   py文件直接运行doTest方法
   css文件将获取@unit-test-expecteds中的属性作为断言
      @unit-test-expecteds中，key为严重级别 0/1/2，value为具体的错误信息文本
'''
...
def doCheckWithPythonFile(f):
    ...
    plugin = __import__("tests." + caseName, fromlist = [caseName])
    pluginMethod = None
...
def runUnitTests():
    global fileCounter
    filePath = realpath(__file__, '../tests')
